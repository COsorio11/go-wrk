...
func NewLoadCfg(duration int, //seconds
	...
	caCert string,
	http2 bool,
	insecure bool) (rt *LoadCfg) {
	rt = &LoadCfg{duration, goroutines, testUrl, reqBody, method, host, header, statsAggregator, timeoutms,
		allowRedirects, disableCompression, disableKeepAlive, 0, clientCert, clientKey, caCert, http2, insecure}
	return
}
...
func (cfg *LoadCfg) RunSingleLoadSession() {
	...
	httpClient, err := client(cfg.disableCompression, cfg.disableKeepAlive, cfg.timeoutms, cfg.allowRedirects, cfg.clientCert, cfg.clientKey, cfg.caCert, cfg.http2, cfg.insecure)
	if err != nil {
		log.Fatal(err)
	}
